...
from .forms import CuastomUserCreationForm, OrdenForm, ClienteForm, EmpresaForm, SolicitudForm, \
    ContenedorForm, BuqueForm, ServicioForm, ImagenesForm, PersonaForm, VendedorForm, CompradorForm, ReceptorForm
from .models import Empresa, Solicitud, Cliente, Buque, Contenedor, Persona, Servicios, Orden, Imagenes
...
def listar_empresas(request):
    ...
    return render(request, 'operaciones/Empresas/Listar_Empresas.html', data)


@login_required
def eliminar_empresa(request, id):
    empresa = get_object_or_404(Empresa, codigo_one=id)
    empresa.delete()
    return redirect(to="listar_empresas")
...
def nueva_empresa(request):
    ...
    if request.method == 'POST':
        ...
        if formulario.is_valid():
            formulario.save()
            messages.success(request, "Se ha creado la empresa correctamente")
            return redirect(to="listar_empresas")
        data = {
            "form": formulario
        }

    return render(request, 'operaciones/Empresas/Nueva_Empresa.html', data)
...
def modificar_empresa(request, id):
    ...
    if request.method == 'POST':
        ...
        if formulario.is_valid():
            formulario.save()
            messages.success(request, "Se ha actualizado la empresa")
            return redirect(to="listar_empresas")
        data["form"] = formulario
    return render(request, 'operaciones/Empresas/Modificar_Empresa.html', data)
